...
import { createPerseusController } from "./core/perseus-controller.ts";
...
export async function main(args: string[], options?: MainOptions) {
	...
	const createRuntime: CreateAgentSessionRuntimeFactory = async ({
		...
		let speculativeActions: CreateAgentSessionOptions["speculativeActions"];
		if (process.env.PERSEUS_ENABLED === "1") {
			const actorModel = sessionOptions.model;
				...
				speculativeActions = createPerseusController({
					...
					maxTokens:
						Number.isFinite(configuredSpeculatorMaxTokens) && configuredSpeculatorMaxTokens > 0
							...
							: undefined,
					timeoutMs:
						Number.isFinite(configuredSpeculatorTimeoutMs) && configuredSpeculatorTimeoutMs > 0
							? configuredSpeculatorTimeoutMs
							: undefined,
					safeTools,
					traceFile: process.env.PERSEUS_TRACE_FILE,
				});
			}
		}
