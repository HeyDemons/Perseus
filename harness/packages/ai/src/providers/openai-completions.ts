		...
		if (msg.role === "user") {
			...
			if (toolCalls.length > 0) {
				...
				if (compat.thinkingFormat === "deepseek") {
					// DeepSeek-compatible chat endpoints can reject the following
					// tool result when the replayed assistant tool-call message also
					// carries non-empty thinking text in content. Keep the tool-call
					// predecessor as a plain tool-call envelope, while preserving
					// reasoning_content when thinking mode is enabled.
